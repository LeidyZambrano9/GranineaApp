...
object UsuarioRepository {
    ...
    // ✅ ACTUALIZADO: Agregamos el parámetro 'celular' obligatorio
    suspend fun actualizarPerfil(
        nombres: String,
        apellidos: String,
        correo: String,
        celular: String, // 👈 Nuevo parámetro
        fotoUrl: String? = null
    ) {
        val userId = SupabaseClient.client.auth
            .currentUserOrNull()?.id ?: return

        // 🛠️ Revisa que las claves ("nombres", "celular", etc.) se llamen EXACTAMENTE
        // igual a las columnas que creaste en la tabla de tu consola de Supabase.
        val datos = buildJsonObject {
            put("nombres", nombres)
            put("apellidos", apellidos)
            put("correo", correo)
            put("celular", celular) // ✅ AGREGADO: Ahora sí viaja el celular al update de Supabase
